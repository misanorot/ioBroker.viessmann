...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'misanorot';
    var email     = grunt.option('email')  || 'audi16v@gmx.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new viessmann name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/viessmann.png')) {
            fs.renameSync(__dirname + '/admin/viessmann.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/viessmann.html')) {
            fs.renameSync(__dirname + '/widgets/viessmann.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/viessmann/js/viessmann.js')) {
            fs.renameSync(__dirname + '/widgets/viessmann/js/viessmann.js', __dirname + '/widgets/viessmann/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/viessmann')) {
            fs.renameSync(__dirname + '/widgets/viessmann',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Viessmann/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Viessmann'
                        },
                        {
                            match: /misanorot/g,
                            replacement: author
